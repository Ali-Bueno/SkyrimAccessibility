# Skyrim Accessibility Mod By Dio Kyrie

## Available Features
- Intro Auto Walkthrough;
- Clairvoyance. A Guiding spell which will lead you to current quest marker via sound trail.
- Sound Notify if Walking in Place.[Work In Progress]
- Audio Markers on Items, NPC, Containers, Doors, etc.[Work In Progress]

## Hotkeys
- X Key - Lock Camera On Target. List of targets to lock camera on.[Work In Progress]
- H Key - Autowalk. Selet Neraby Target, Interest Target or City.[Work In Progress]
- V Key - Activate Menu. Interact with NPC, Doors, Containers, etc.Max Distance 10 Meters.[Work In Progress]
- G Key - LockPicking. Try to pick All Locks in Radius of 2 Meters.
- U Key - None
- O Key - Toggle Clairvoyance. Turns On or Off Clairvoyance Autocast.
- B Key - Character Status. Health, Mana, Stamina, Debuffs.[Work In Progress]
- K Key - None
- L Key - Debug Menu. Write Coordinates and Make ScreenShot.

## Install Guide
1. Install [Skyrim Special Edition](https://store.steampowered.com/app/489830/The_Elder_Scrolls_V_Skyrim_Special_Edition/);
2. Install [Mod Manager](https://github.com/DioKyrie-Git/SkyrimAccessibility/blob/main/ModManagersGuides.md);
3. Launch Skyrim Launcher for it to automatically generate config files and close it;
4. Download and Install Requirements;
5. Install latest release of [Skyrim Accessibility](https://github.com/DioKyrie-Git/SkyrimAccessibility/releases) mod;
6. Install latest release of [SkyrimTTS](https://github.com/DioKyrie-Git/SkyrimTTS/releases);
7. Launch SKSE from within MO2.

### Requirements:
- [SKSE](https://www.nexusmods.com/skyrimspecialedition/mods/30379?tab=files);
- [Addres Library for SKSE plugins](https://www.nexusmods.com/skyrimspecialedition/mods/32444?tab=files);
- [Power Of Three's Papyrus Extender](https://www.nexusmods.com/skyrimspecialedition/mods/22854?tab=files);
- [UIExtensions](https://www.nexusmods.com/skyrimspecialedition/mods/17561?tab=files);
- [Media Keys Fix](https://www.nexusmods.com/skyrimspecialedition/mods/92948?tab=files);
- [Better MessageBox Controls](https://www.nexusmods.com/skyrimspecialedition/mods/1428?tab=files);
- [Stay At The System Page - AE](https://www.nexusmods.com/skyrimspecialedition/mods/67883?tab=files).


## Licenses Notice
Files of Skyrim Accessibility Mod By Dio Kyrie are licensed under different licenses.
You can learn more in License.md
